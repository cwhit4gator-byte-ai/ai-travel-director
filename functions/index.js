...
Give every itinerary item a location containing its city or town. Set each day’s overnightLocation to the location of that day’s final itinerary item. For every day before the trip ends, recommend the geographically practical city or district for that overnight stay. Use the same location on consecutive nights when moving would add needless friction. Include every unique stop in overnightLocations. Limit the trip to 10 days and 5 items per day. Use numeric estimated costs only when useful and label uncertainty in the message.`,
